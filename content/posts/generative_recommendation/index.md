...
1. *Generative Recommendation probably isn't a fad*: It's too early to say whether Generative Recommendation is the "ChatGPT moment" for RecSys, but it probably isn't a fad --- many companies have hit diminishing returns with DLRM models (an exception: [RankMixer](https://arxiv.org/html/2507.15551v1) from ByteDance keeps pushing the boundary of feature interaction modules) and do need scaling laws to keep improving model performance. So far GM offers the best bet to achieve that.
